...
if (isset($_POST['login'])) {
    $key = 'soleniehasla';
    $password = md5($key.'-'.$_POST['password']);
    $result = $pdo->prepare('SELECT * FROM users WHERE login = :login AND password = :password AND attempts < 3');
    ...
    if ($user) {
        ...
        $send = $pdo->prepare('UPDATE `users` SET `attempts` = 0 WHERE `login` = :login');
        $send->bindParam(':login', $_POST['login']);
        $send->execute();
        header('location: index.php?v=categories');
    } else {
        echo '<h1>Podano złe dane logowania</h1>';
        $result = $pdo->prepare('SELECT * FROM users WHERE login = :login');
        $result->bindParam(':login', $_POST['login']);
        $result->execute();
        $user = $result->fetch();
        if ($user) {
            $send = $pdo->prepare('UPDATE `users` SET `attempts` = attempts+1 WHERE `login` = :login');
            $send->bindParam(':login', $_POST['login']);
            $send->execute();
        }
    }
...
<form method="post">
    ...
    <p class="text-center pointer"><input class="check_box" type="checkbox" id="read" 
    onClick="$('#password').attr('type', this.checked ? 'text' : 'password');
    $('#img').attr('src', this.checked ? 'images/login/service.png' : 'images/login/service_off.png');
    $('#txt').text(this.checked ? 'FBI AGENT MODE ON' : 'FBI AGENT MODE OFF')"/> 
    <label for="read"><img id="img" style="width:50px;display:inline" src="images/login/service_off.png" /></label>
    <label id="txt" style="display:inline;select:none;" for="read">FBI AGENT MODE OFF</label></p>
	<div class="inputGroup inputGroup1">
		<label for="email1">Email</label>
		<input type="text" id="email" name="login" class="email" maxlength="256"/>
